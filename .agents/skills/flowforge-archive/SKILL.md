...
`archive-synthesize` 输出的 JSON 合成计划中包含 `maturityChanges` 段。

对每个 maturity 变更：
- **升级**（growing → stable）：被当前 proposal 引用验证的 library 文档 → 更新 frontmatter: `maturity: stable`
- **降级**（stable → deprecated）：被当前 proposal 推翻的 library 文档 → 更新 frontmatter: `maturity: deprecated`，同时 `related.ref` 指向新文档
- **新建**：新文档 maturity 默认 `growing`（proposal 提取的有实质内容，不是 seed）
