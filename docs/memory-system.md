...
2. **文档腐烂与知识孤岛（Living vs Dead Docs）**：传统的系统文档在项目演化中迅速脱节，提案沉淀的知识成为孤岛。
...
│  路径: docs/CONTEXT.md (或 .flowforge/PROJECT_MEMORY.md)   │
│  内容: 系统架构常识、长期技术约束、全局统一语言、活跃提案索引│
└──────────────────────────────┬──────────────────────────────┘
                               │ 注入全局背景
┌──────────────────────────────▼──────────────────────────────┐
│  Tier 2: 提案级活笔记 (Proposal Working Scratchpad)          │
│  路径: 01-workspace/<proposal_id>/README.md                 │
│  内容: 提案目标、Grill共识、调查事实链、未决问题、Tracer切片│
└──────────────────────────────┬──────────────────────────────┘
                               │ 提取切片最小上下文
┌──────────────────────────────▼──────────────────────────────┐
│  Tier 3: 任务级执行切片 (Slice Execution Context)           │
│  机制: 动态装配 (通过 CLI 或 Skill 提取)                    │
...
* **主要内容**：
  ...
  * **Architectural Constraints**：不可逾越的技术选型与工程规范（如：数据库选型、分层架构、TDD 纪律）。
  * **Active Proposals Index**：当前进行中的提案索引。
* **生命周期**：全局唯一，任何 Agent 会话启动时默认加载。

### 2.2 Tier 2: 提案级活笔记 (`01-workspace/<proposal_id>/README.md`)
* **定位**：长周期讨论的**外脑（External Scratchpad）**。在需求探讨的第一分钟即创建，随着人机对话不断精炼追加。
* **自适应规模与渐进式披露（Adaptive Scale & Progressive Disclosure）**：
  * **单文件极简模式（Lean Mode）**：适用于中小型任务（单一业务域、<= 5 个切片），所有目标、共识、事实与切片全部内聚在 `README.md` 中。
  * **多层总分模式（Hierarchical Mode）**：当命中复杂特征（跨多个独立子模块、长篇网络/调研事实 > 500字、或切片 > 6 个）时，Agent 自动平滑升级为总分结构：
    ```text
    01-workspace/<proposal_id>/
    ├── README.md           # 总控大纲与架构中枢 (Hub)
    ├── references/         # 深度调研与外部知识 (Web/API/Schema)
    ├── modules/            # 子模块详细设计 (Detailed Specs)
    └── JOURNAL.md          # 关键演进流水
    ```
* **核心章节**：
  1. `Objective`：核心业务/技术目标（1~2 句话）。
  2. `Ubiquitous Language`：本提案专属的名词定义（消除歧义）。
  3. `Explored Facts`：调查代码/数据得到的权威事实（必须附 `path/file:line`，长篇调研链接至 `references/`）。
  4. `Key Decisions & Consensus`：已达成共识的技术方案（记录 Why，复杂子模块拆解至 `modules/`）。
  5. `Open Questions`：待对齐问题清单（跨会话优先追问）。
  6. `Actionable Slices`：确认后的 3~6 个 Tracer Bullet 切片清单及绑定的自动化测试。
* **断点续传机制**：跨会话时，Agent 优先读取此文件，无需用户重新复述背景，即可 100% 恢复讨论状态。

### 2.3 Tier 3: 任务级执行切片 (Slice Context)
* **定位**：执行具体代码改造时的瞬时上下文。
* **机制**：通过 `flowforge context slice --slice <n>` 动态组装，仅包含当前切片的目标、接缝文件与测试命令，确保最大、最干净的代码上下文窗口。
