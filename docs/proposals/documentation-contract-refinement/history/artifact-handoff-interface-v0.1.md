# Artifact Hand-off Interface v0.1

**Date:** 2026-08-25  
**Status:** Approved interface baseline; schemas and production Skills not yet implemented  
...
Each independently authoritative item may carry a stable semantic slug and lightweight semantic revision:

```yaml
flowforge:
  id: provider-construction-owner
  revision: 3
```
...
When current revision exceeds consumed revision, emit `upstream-changed` as a warning and identify the affected consumer. After review, the consumer records the revision it now understands.
...
1. Re-run the simple and complex scenarios against this interface.
2. Decide the minimal physical schema and parser role marker only after the walkthrough passes.
3. Design production Skill changes for align, solution-design, plan, implement, and review.

The scenario run is captured in [Artifact Hand-off Scenario Validation v0.3](artifact-scenario-validation-v0.3.md) and the [throwaway v3 interaction prototype](workflow-prototype-v3.throwaway.html). Its proposed refinements remain experimental until approved.
