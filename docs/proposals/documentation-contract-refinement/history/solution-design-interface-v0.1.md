# `flowforge-solution-design` Interface Design v0.1

**Date:** 2026-08-25  
**Status:** Approved interface baseline; Skill implementation not yet designed  
...
- the behavior requires coordination across two or more modules;
...
3. a coverage report explaining which planning-relevant facts are settled;
...
Before recommending planning, derive and report whether:
...
The report identifies evidence and gaps. It does not store a readiness state.
...
The metadata shape is illustrative, not an approved schema.
...
- Final Markdown schema and metadata syntax.
...
- Writing the production `SKILL.md` before this interface is reviewed against the two workflow scenarios.

## Next design work

1. Apply this interface to the corrected simple-feature walkthrough and confirm that the Skill is skipped.
2. Apply it to the Tangram-style complex walkthrough and inspect the resulting design artifact shape.
3. Design the minimum requirement, ticket, and evidence interfaces at the hand-off seams.
4. Only then write the production Skill instructions and supporting references.
