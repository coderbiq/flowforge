...
| **知识库查询** | `library <action>` | 发现 library facets、分类任务、推荐规范/模块/历史设计 |
...
FlowForge 不固定项目 taxonomy。项目通过 library 卡片 tags 表达 facet：
...
`library facets` 从 library 卡片中统计可用 facet key/value 和常见组合，供 Agent 先理解当前项目知识组织方式。

`library classify --for` 使用已发现的 facet vocabulary，从任务、需求或设计卡的 title、tags、domain、body 中提取候选 facet。该命令只输出建议，不写入任务卡。
