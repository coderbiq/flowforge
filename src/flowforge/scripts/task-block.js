...
if (!proposalId || !taskId) {
  console.error('Usage: task-block.js <projectRoot> <proposalId> <taskId> [reason]');
  process.exit(1);
}
...
async function main() {
  ...
  if (!proposalDir) {
    console.log(JSON.stringify({ blocked: false, error: `proposal ${proposalId} not found` }));
    process.exit(0);
  }

  const result = await adapter.blockTask(proposalDir, taskId, reason);
