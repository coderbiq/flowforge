...
1. 创建 `task-map.yaml`，为需求树中**已知的叶子节点**创建对应的 analysis 任务：
...
每个 analysis 任务的 `epic` 指向需求树中的父分支（如 `auth`、`session`），`title` 直接对应需求树叶子节点的描述。
...
│        ↓ 发现新需求点                                     │
│  补充到需求树 + task-add.js 创建新 analysis 任务           │
│        ↓ 该需求分析充分                                   │
│  task-add.js 创建 design 任务（sourceTasks 指向分析任务）  │
│        ↓ 需求树该节点确定（移除 [?]）                      │
│                     ↓                                     │
...
3. task-map.yaml 中所有 `design` 类型任务状态为 `done`
