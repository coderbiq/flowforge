...
sets up <docs_dir>/agents/ rules, creates .flowforge/ configuration, and sets up the Wiki hierarchy.`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if _, err := os.Stat(configFile); os.IsNotExist(err) {
				defaultYAML := fmt.Sprintf("version: 5.0.0\nversion_check: true\ndocs_dir: %s\n", config.DefaultDocsDir)
			...
			initHosts, err := resolveHostTargets(cfg)
			if err != nil {
				return fmt.Errorf("deploying subagents: %w", err)
			}
			...
			fmt.Fprintf(cmd.OutOrStdout(), "✓ Deployed flowforge subagents to %s\n", describeHostDirs(initHosts))
