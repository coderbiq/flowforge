...
const { beadTaskSummary, loadProposalContext, validateProposalContext } = require('./lib/flowforge');

function main() {
  const target = process.argv[2];
  if (!target) {
    console.error('Usage: scripts/flowforge-proposal-status.js <proposal-id|proposal-dir>');
    process.exit(2);
  }
  ...
  try {
    ...
    const summary = {
      ...
      workspace: context.meta.workspace || null,
