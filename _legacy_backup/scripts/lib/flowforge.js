...
function loadProposalContext(target, cwd = process.cwd()) {
  ...
  const explorations = [];
  if (meta.schema_version === 'v2' && Array.isArray(meta.source_explorations)) {
    for (const source of meta.source_explorations) {
      if (!source?.workspace || !source?.ref) continue;
      const explorationRoot = getWorkspaceDocsRoot(source.workspace, cwd);
      const explorationDir = path.join(explorationRoot, 'explorations', source.ref);
      ...
      if (fileExists(indexPath)) {
        explorations.push({
          ...
          dir: explorationDir,
          indexPath,
          indexContent: readFileRequired(indexPath),
        });
      }
    }
  }
  ...
  if (schemaVersion === 'v2') {
    ...
    if (!Array.isArray(meta.archive_targets) || meta.archive_targets.length === 0) {
      ...
      for (const target of meta.archive_targets) {
        ...
        if (target?.type === 'module' && target?.ref && !target.ref.startsWith('modules/')) {
          errors.push(
            `module archive target "${target.key || target.ref}" ref is "${target.ref}" ` +
            `— must start with "modules/" (canonical module path)`
          );
        }
      }
    }
...
function createProposalSkeleton(options, cwd = process.cwd()) {
  ...
  const archiveTargets = (options.archiveTargets || []).map((target, index) => {
    ...
    const rawRef = target.ref || target.path;
    const ref = target.type === 'module' && rawRef && !rawRef.startsWith('modules/')
      ? path.join('modules', rawRef).split(path.sep).join('/')
      : rawRef;
...
function ensureModuleArchiveTarget(targetPath, context, cwd = process.cwd()) {
  ...
  if (context.explorations && context.explorations.length > 0) {
    const explorationLinks = context.explorations
      .map(exp => `- [${exp.ref}](${toPosixPath(path.relative(moduleDir, exp.indexPath))})`)
      .join('\n');
    if (explorationLinks) {
      appendText(readmePath, `\n## Related explorations\n\n${explorationLinks}\n`);
    }
  }
...
function ensureArchiveTarget(target, context, cwd = process.cwd()) {
  ...
  if (target.type === 'module') {
    if (target.ref && !target.ref.startsWith('modules/')) {
      throw new Error(
        `module archive target ref must start with "modules/", got "${target.ref}". ` +
        `Fix meta.yaml archive_targets or re-create the proposal.`
      );
    }
...
function routeSourceContentToModule(context, moduleDir, cwd = process.cwd()) {
  const moduleName = path.basename(moduleDir);
  const docsRoot = getDocsRoot(cwd, context.meta.workspace);
  const moduleRef = toPosixPath(path.relative(docsRoot, moduleDir));

  const sourceDocs = [
    { path: context.designPath, targetFile: 'design.md' },
    { path: path.join(context.proposalDir, 'proposal.md'), targetFile: 'README.md' },
  ];

  for (const { path: sourcePath, targetFile } of sourceDocs) {
    if (!fileExists(sourcePath)) continue;

    const sourceContent = readFileRequired(sourcePath);
    const { frontmatter, body } = parseFrontmatterDocument(sourceContent);
    if (!frontmatter?.archive_target) continue;

    let targetRef = String(frontmatter.archive_target);
    const colonIdx = targetRef.indexOf(':');
    if (colonIdx >= 0) {
      targetRef = targetRef.slice(colonIdx + 1);
    }

    const targetBasename = path.basename(targetRef);
    if (targetBasename !== moduleName && targetRef !== moduleRef) continue;

    const trimmedBody = body.trim();
    if (!trimmedBody) continue;

    const targetPath = path.join(moduleDir, targetFile);
    appendText(targetPath, `\n\n${trimmedBody}\n`);
  }
}
...
function archiveProposal(context, cwd = process.cwd()) {
  ...
  for (const target of context.meta.archive_targets || []) {
    const result = ensureArchiveTarget(target, context, cwd);
    if (target.type === 'module' && target.ref) {
      const moduleName = path.basename(target.ref);
      routeSourceContentToModule(context, result.path, cwd);
    }
